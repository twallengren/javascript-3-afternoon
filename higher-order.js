...
//Code Here
let totalPopulation = populations.reduce((tot, elem, ind, arr) => {
  return tot + elem
})
...
//Code Here
let myStrongest = monstersInYourPocket.filter((val, ind, arr) => {
  return val["CP"] > 200
})
...
let ordersTotal = orders.map(elem => (1 + elem["tax"]) * elem["price"])
//let ordersTotal = orderstax.reduce((tot, val) => tot + val)
...
*/
let bobs = purchases.filter((val) => {return val["owner"] === "Bob"})
let bob = bobs.map((elem) => {return elem["price"]})
let bobsTotal = bob.reduce((acc, val) => {return acc + val})
